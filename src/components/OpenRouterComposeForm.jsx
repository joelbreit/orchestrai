			...
			<Form onSubmit={handleSubmit} className="mt-3">
				...
				>
					{Object.keys(MODELS).map((model) => (
						<option key={model} value={MODELS[model]}>
							{model}
						</option>
					))}
				</Input>
				<FormGroup>
					<Label for="vibe" style={{ marginRight: "0.5rem" }}>
					...
					<Button
						...
						onClick={suggestVibe}
					>
						Suggest one for me <i className={`bi bi-magic`}></i>
					</Button>
					<Input
						type="text"
						id="vibe"
						...
						className="mt-2"
					/>
					<span className="character-counter">{vibe.length}/100</span>
				</FormGroup>
				<FormGroup>
					<Button
						type="submit"
			...
			{hasGeneratedMusic && (
				<>
					...
					{vibe || fullPrompt ? (
						<>
							{description ? (
								...
								<p>*No description was generated this time.*</p>
							)}
						</>
					) : (
					...
					{saveState === "Complete" && (
						<Alert
							color={
								saveStatusCode === 200 ? "success" : "danger"
							}
						>
							{saveStatusCode === 200 ? (
								<>
									Your tune was saved successfully! You can
									now share it with{" "}
									<Link
										to={`/tunes/${tuneId}`}
										target="_blank"
							...
							<Alert color="info">
								...
								>
									...
									<span>
										...
										repaired. {warnings.length} warnings
										were issued. See something strange? Send
										a message to the{" "}
										<a
...
export default OpenRouterComposeForm;
