...
// const filePath = `${__dirname}/private/UncleanedNotation.abc`;
const filePath = `${__dirname}/private/AllCreations.abc`;
	...
	calculateLength() {
		if (this.text.match(this.regex.valid)) {
			...
			if (this.text === "") {
				...
				this.addWarning(
					"Unrecognized note length: " + this.text
				);
			}
		} else {
...
class Note {
	...
	parseNote() {
		...
		if (this.text.match(this.regex.valid)) {
			const [, articulation, accidental, noteLetter, octaveModifiers, noteLength, tie] = this.text.match(this.regex.valid);
...
class Measure {
	...
	parse() {
		...
		for (let i = 0; i < this.text.length; i++) {
			...
			if (noteFound) {
				if (currentChar.match(/[#\d/>,'-]/)) { // # not valid, but it's easier to just overlook it
					// note length
					part += currentChar;
					// broken rhythms
					if (currentChar === ">") {
						if (brokenRhythm.relation === "after") {
							this.addWarning("Broken rhythm before and after note");
						}
						brokenRhythm.value += ">";
					...
					if (brokenRhythm.value) {
						lengthMultiplier *= brokenRhythms[brokenRhythm.relation][brokenRhythm.value];
					}
					this.parts.push(new Note(preText, part, postText, this, lengthMultiplier));
				...
				if (currentChar.match(/[.vu~=_^]/i)) {
					...
					if (remainingText[1].match(/\d/i)) {
						...
						if (tuplet === "(3") {
							...
							this.addWarning("Tuplet other than triplet not yet supported: " + remainingText);
						}
						preText += tuplet;
						i += tupletLength - 1;
					} else {
					...
					const chordName = remainingText.match(this.regex.chordName)[0];
					const chordLength = chordName.length;
					preText += chordName;
					i += chordLength - 1;
				} else if (currentChar === '{') {
					// grace notes
					const graceNotes = remainingText.match(this.regex.graceNotes)[0];
					...
					inChord = true;
					firstChordNote = true;
					const lengthenedChordMatch = remainingText.match(/^\[[^\]]+\](\d+)/i);
					...
					this.addWarning("Unexpected char before note definition: " + currentChar);
				}
			}
		}
		if (noteFound) {
			...
			try {
				this.parts[this.parts.length - 1].postText += preText + part + postText;
			} catch (error) {
				this.addWarning("Error in adding postText to last note: " + error);
			}
		}
	}
	...
	get totalLength() {
		let length = this.parts.reduce((acc, part) => acc + part.totalLength, 0);
	...
	setMeasureLength(desiredLength) {
		...
		if (lengthDifference > 0) {
			...
			if (lengthDifference >= 1) {
				...
				this.addFixes(`Added ${lengthDifference} rests to the end of the measure`);
			}
			if (remainder) {
				switch (remainder) {
					case 0.5:
					...
					case 0.25:
						...
						break;
					default:
						this.addWarning(`Can't add this length to measure: ${lengthDifference}`);
				}
				if (validLength) {
					const newNote = new Note("", textToAdd, "", this);
					this.parts.push(newNote);
			...
			if (remainingDifference !== 0) {
				...
				this.addFixes(`Removed ${-lengthDifference} notes from the end of the measure`);
			}
		}
	...
	parse() {
		...
		for (let i = 0; i < this.text.length; i++) {
			...
			if (!inMeasure || beginningOfLine) {
				if (remainingText.match(this.regex.singleBarline || remainingText.match(this.regex.doubleBarline))) {
					inMeasure = true;
					preText += currChar;
		...
		if (this.measures.length === 0) {
			...
			this.measures[this.measures.length - 1].postText += preText + measure + postText;
		}
	}

	get fullText() {
		const measuresText = this.measures.map((measure) => measure.fullText).join("");
		return this.preText + measuresText + this.postText;
	}

	get totalLength() {
		return this.measures.reduce((acc, measure) => acc + measure.totalLength, 0);
	}
...
class Voice {
	...
	parse() {
		...
		for (const line of lines) {
			if (line.match(REGEX.commentLine) || line.match(REGEX.lyricLine) || line.match(REGEX.voiceDefinition || line.match(REGEX.partDefinition))) {
				preText += line + "\n";
			} else if (line !== "") {
	...
	get measureLengths() {
		...
		for (const line of this.lines) {
			lengths = lengths.concat(line.measures.map((measure) => measure.totalLength));
		}
		return lengths;
	}
}
...
class ABCNotation {
				...
				for (const line of voice.lines) {
					for (const measure of line.measures) {
						if (measure.totalLength !== desiredMeasureLength) {
							...
							const otherVoices = this.parts.filter((part) => part.index !== voice.index);
							const otherMeasures = otherVoices.map((part) => part.lines[lineIndex].measures[measureIndex]);
							const areSameLength = otherMeasures.every((otherMeasure) => otherMeasure.totalLength === measure.totalLength);
							if (areSameLength) {
								this.addNote("Not enforcing measure length for line " + lineIndex + ", measure " + measureIndex);
							} else {
	 ...
	 */
	parseHeaders() {
		const headerSection = this.abcNotation.match(REGEX.headers.entireSection);
	...
	determineDesiredMeasureLength() {
		...
		const interpretMeterAndLength = () => {
			try {
				...
				const timeSignature = this.meter.match(REGEX.meterDefinition)[1];
				if (!timeSignature) {
				...
				if (timeSignature !== "C") {
					if (timeSignature === "C|") {
						...
						timeNumorator = this.meter.match(REGEX.meterDefinition)[2];
						timeDenominator = this.meter.match(REGEX.meterDefinition)[3];
					}
				}
				...
				if (this.length) {
					// get note length
					[lengthDefinition, noteLength, noteDenominator] = [...this.length.match(REGEX.lengthDefinition)];
					// const noteLength = this.length.match(REGEX.lengthDefinition)[1];
					if (noteLength !== "1") {
						this.addWarning("Unexpected denominator of note length: " + lengthDefinition);
					}
				...
				const notesPerMeasure = timeNumorator * (noteDenominator / timeDenominator);
				...
				this.addWarning("Error in interpreting meter and length: " + error);
				return 8;
			}
		}
	...
	checkForErrors() {
		try {
			...
			for (let i = 0; i < lengthsMatrix[0].length; i++) {
				...
				if (measureLengths.some((length) => length !== measureLengths[0])) {
					this.addWarning(`Measure ${i} has mismatched lengths: ${measureLengths}`);
				}
			}
		} catch (error) {
	...
	setReferenceNumber(number) {
		...
		this.headerText = this.headerText.replace(REGEX.referenceNumber, newReference);
	}
	...
	setOrigin(origin) {
		...
		if (this.headerText.match(REGEX.origin)) {
			...
			this.headerText = this.headerText.slice(0, kIndex) + newOrigin + "\n" + this.headerText.slice(kIndex);
		}
	}
	...
	setHeader(H, value) {
		...
		if (this.headerText.match(customRegex)) {
			...
			// Place before the first K: line
			const kIndex = this.headerText.indexOf("K:");
			this.headerText = this.headerText.slice(0, kIndex) + newValue + "\n" + this.headerText.slice(kIndex);
		}
	}
}
...
const test = new ABCNotation(testTune);

const lengths = test.lengths;
const fullText = test.fullText;

console.log(test);

// read ../../assets/UncleanedNotation.abc as a text file
// const filePath = path.join(__dirname, '../../assets/UncleanedNotation.abc');
const data = await readFile(filePath, 'utf-8');
await writeFile(`${__dirname}/private/clean.abc`, "");

// split on X:
const tunes = data.split("X:");

let numClean = 0;
let numFailed = 0;
for (let i = 1; i < tunes.length; i++) {
	const abcNotation = "X:" + tunes[i];
	try {
		const test = new ABCNotation(abcNotation);
		const result = new ABCNotation(test.fullText);
		const fullText = result.fullText;
		const width = 50;
		if (result.warnings.length > 0) {
			console.warn(`Tune #${i} - ${result.title.substring(0, width)} has ${result.warnings.length} warnings`);
			numFailed++;
		} else {
			console.log(`Tune #${i} - ${result.title.substring(0, width)} has was successful and made ${test.fixes.length} fixes`);
			numClean++;

			// Write result.fullText to ./private/clean.abc
			result.setReferenceNumber(numClean);
			result.setOrigin("OrchestrAI");
			const cleanFilePath = `${__dirname}/private/clean.abc`;
			const text = result.fullText + "\n\n";
			await appendFile(cleanFilePath, text);
		}
	} catch (error) {
		console.error(`Error in tune #${i}: ${error}`);
		numFailed++;
	}
}

console.log(`Cleaned ${numClean} tunes and failed to clean ${numFailed} tunes`);